...
namespace X
{
	void CApplication::initOnce(void)
	{
		...
		pWindow->setText("X: DemoOctTree. F1: Toggle fullscreen. F2: Toggle Vsync. F3: Toggle statistics window.");
...
//		SCGUIManager::getPointer()->getContainer("X:Default:Statistics")->setVisible(true);
		...
		// Cube triangle
		CResourceVertexBufferCPT* pVB = pRM->addVertexBufferCPT("cube1x1x1");
		...
		// Ground triangle
		pVB = pRM->addVertexBufferCPT("ground");
