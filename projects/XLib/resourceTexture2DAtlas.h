...
namespace X
{
	...
	public:
		...
		void bind(unsigned int uiTextureUnit = 0, unsigned int uiImageNumber = 0);
		...
		void bind(unsigned int uiTextureUnit, const std::string& strImageName);

		// Binds the specified atlas texture index
		// uiTextureUnit should be between 0-7
		// uiAtlasImageNumber should be a valid image number otherwise an exception occurs
		void bindAtlas(unsigned int uiTextureUnit = 0, unsigned int uiAtlasImageNumber = 0);
		...
		unsigned int getNumImages(void);
		...
		void getTextureCoords(unsigned int uiImageNumber, glm::vec2& vTCMin, glm::vec2& vTCMax);
		...
		void getTextureCoords(const std::string& strImageName, glm::vec2& vTCMin, glm::vec2& vTCMax);
		...
		// If an invalid image number is given, an exception occurs.
		std::string getImageFilename(unsigned int uiImageNumber);

		// Returns whether when adding the image to the atlas, it was rotated or not
		// If an invalid image number is given, an exception occurs.
		bool getImageWasRotated(unsigned int uiImageNumber);

		// Returns whether when adding the image to the atlas, it was rotated or not
		// If an invalid image name is given, an exception occurs.
		bool getImageWasRotated(const std::string& strImageName);

		// Returns which atlas image the given image number is stored in
		// If an invalid image number is given, an exception occurs.
		unsigned int getImageStoredInAtlasNumber(unsigned int uiImageNumber);

		// Returns which atlas image the given image name is stored in
		// If an invalid image name is given, an exception occurs.
		unsigned int getImageStoredInAtlasNumber(const std::string& strImageName);

		// Returns image name at specified index;
		// If an invalid index is given, an exception occurs
		std::string getImageNameAtIndex(unsigned int uiImageNumber);
