...
namespace X
{
	...
	void GUIButtonImage::render(void* pParentContainer, const std::string& strFramebufferToSampleFrom)
	{
		...
		renderBackground(pParentContainer, strFramebufferToSampleFrom, pTheme->mImages.buttonImageBGColour, pTheme->mImages.buttonImageBGNormal);
		...
		if (state::over == _mState)
			...
			pTexColour = pRM->getTexture2D(_mstrTextureDown);
