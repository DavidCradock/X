...
namespace X
{
	...
	void CDemoNeuralNetsState3::onEnter(void)
	{
		// Create materials
		CSMMaterial* pMat;
		// Critter material
...
//		pMat->mstrTextureNameDiffuse = "critter_diffuse";
		...
		for (int i = 0; i < iNumberOfEntities; i++)
		{
			...
			pEnt = _mSM.addEntityVertexBuffer(strName, "critter", "entity");
	...
	void CDemoNeuralNetsState3::onActive(CFiniteStateMachine* pFSM)
	{
		_mTimer.update();
