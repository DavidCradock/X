...
namespace X
{
	...
	void InputManager::init(HWND hApplicationWindow)
	{
		...
		ThrowIfTrue(!instance, "InputManager::init() failed. Unable to obtain application instance handle.");
		...
		if (DI_OK != DirectInput8Create(instance, DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&directInput, NULL))
			ThrowIfTrue(1, "InputManager::init() failed. Unable to create main DirectX input object.");

		// Init joystick
		if (!joy.init(directInput, windowHandle))// Error only returned if joystick detected, but init failed.
			ThrowIfTrue(1, "InputManager::init() failed. Joystick detected, but unable to initialise it.");

		// Init keyboard
		if (!key.init(directInput, windowHandle))
			ThrowIfTrue(1, "InputManager::init() failed. Unable to initialise keyboard.");

		// Init mouse
		if (!mouse.init(directInput, windowHandle))
			ThrowIfTrue(1, "InputManager::init() failed. Unable to initialise mouse.");
	}
