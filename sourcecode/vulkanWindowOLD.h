...
namespace X
{
	...
	class VulkanWindowOLD : public Singleton<VulkanWindowOLD>
	{
	public:
		VulkanWindowOLD();
	...
	private:
		...
		std::vector<VkCommandBuffer> mvkCommandBuffers;			// Command buffers (No need to destroy this as it is when command pool is)
		
		std::vector<VkSemaphore> mvkSemaphoresImageAvailable;	// GPU syncronization semaphore
		std::vector<VkSemaphore> mvkSemaphoresRenderFinished;	// GPU syncronization semaphore
		std::vector<VkFence> mvkFencesInFlight;					// GPU syncronization fence
		uint32_t muiCurrentFrame = 0;							// Keep track of what frame we're using

		void _createSwapchain(void);
		void _createImageViews(void);
		void _createFramebuffers(void);

		// Recreate the swapchain due to window resizing
