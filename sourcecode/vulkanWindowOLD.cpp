...
namespace X
{
	...
	{
		return VulkanWindowOLD::getPointer()->MsgProc(hwnd, msg, wParam, lParam);
	}
	...
	VulkanWindowOLD::VulkanWindowOLD()
	{
	...
	void VulkanWindowOLD::initialise(std::string strWindowTitle, int iWindowWidth, int iWindowHeight, bool bVsyncEnabled, bool bLogLogicalDeviceExtensions)
	{
		Log* pLog = Log::getPointer();
		...
		allocInfo.commandBufferCount = mvkCommandBuffers.size();
		ThrowIfTrue(VK_SUCCESS != vkAllocateCommandBuffers(mvkLogicalDevice, &allocInfo, mvkCommandBuffers.data()),  "VulkanWindow::initialise() failed to allocate command buffers.");
		
		// Create syncronization objects
		mvkSemaphoresImageAvailable.resize(MAX_FRAMES_IN_FLIGHT);
		mvkSemaphoresRenderFinished.resize(MAX_FRAMES_IN_FLIGHT);
		mvkFencesInFlight.resize(MAX_FRAMES_IN_FLIGHT);
		...
		{
			ThrowIfTrue(VK_SUCCESS != vkCreateSemaphore(mvkLogicalDevice, &createInfoSemaphore, nullptr, &mvkSemaphoresImageAvailable[i]), "VulkanWindow::initialise() failed to create semaphore used to determine whether swapchain image is available.");
			ThrowIfTrue(VK_SUCCESS != vkCreateSemaphore(mvkLogicalDevice, &createInfoSemaphore, nullptr, &mvkSemaphoresRenderFinished[i]), "VulkanWindow::initialise() failed to create semaphore used to determine whether rendering has finished.");
			ThrowIfTrue(VK_SUCCESS != vkCreateFence(mvkLogicalDevice, &createInfoFence, nullptr, &mvkFencesInFlight[i]), "VulkanWindow::initialise() failed to create in flight fence.");
		}
	...
	void VulkanWindowOLD::_createSwapchain(void)
	{
		Log* pLog = Log::getPointer();
		...
		{
			ThrowIfTrue(1, "VulkanWindow::_createSwapchain() detected that the Vulkan physical device and it's created window surface have no formats.");
		}
		pLog->add("VulkanWindow::_createSwapchain() has detected " + std::to_string(formatCount) + " formats available.");
		...
		ThrowIfFalse(bFormatFound, "VulkanWindow::_createSwapchain() was unable to find the requested format of VK_FORMAT_B8G8R8A8_SRGB.");
		pLog->add("VulkanWindow::_createSwapchain() found the requested format of VK_FORMAT_B8G8R8A8_SRGB.");
		...
		{
			ThrowIfTrue(1, "VulkanWindow::_createSwapchain() detected that the Vulkan physical device and it's created window surface have no presentation modes.");
		}
		pLog->add("VulkanWindow::_createSwapchain() has detected " + std::to_string(presentModeCount) + " presentation modes available.");
		...
		ThrowIfFalse(bPresentationModeFound, "VulkanWindow::_createSwapchain() was unable to find the requested presentation mode.");
		pLog->add("VulkanWindow::_createSwapchain() was able to find the requested presentation mode.", false, true);
		if (mbVsyncEnabled)
		...
		{
			VkExtent2D actualExtent = { static_cast<uint32_t>(miWindowWidth), static_cast<uint32_t>(miWindowHeight) };
		...
		pLog->add("VulkanWindow::_createSwapchain() attempting to create swapchain... ", false, true);
	...
	void VulkanWindowOLD::_createImageViews(void)
	{
		Log* pLog = Log::getPointer();
		...
		pLog->add("VulkanWindow::_createImageViews() attempting to create image views for the swapchain images...", false, true);
		for (size_t i = 0; i < mvkSwapchainImages.size(); i++)
		{
			...
			{
				ThrowIfTrue(1, "VulkanWindow::_createImageViews() failed to create image views for the swapchain images");
			}
		}
		pLog->add(" done.", true, false);
	}

	void VulkanWindowOLD::_createFramebuffers(void)
	{
		...
		for (size_t i = 0; i < mvkSwapchainImageViews.size(); i++)
		{
			...
			ThrowIfTrue(VK_SUCCESS != vkCreateFramebuffer(mvkLogicalDevice, &createInfoframebuffer, nullptr, &mvkSwapchainFramebuffers[i]), "VulkanWindow::_createFramebuffers() failed to create framebuffer.");
		}
		// Set stored dims to members
		miWindowWidth = mvkSwapchainExtent.width;
		miWindowHeight = mvkSwapchainExtent.height;
	}

	void VulkanWindowOLD::_createDefaultGraphicsPipeline(void)
	{
	...
	void VulkanWindowOLD::_recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex, const std::string &strGraphicsPipelineName, const std::string &strGraphicsPipelineGroupName)
	{
	...
	void VulkanWindowOLD::shutdown(void)
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			vkDestroySemaphore(mvkLogicalDevice, mvkSemaphoresImageAvailable[i], nullptr);
			vkDestroySemaphore(mvkLogicalDevice, mvkSemaphoresRenderFinished[i], nullptr);
			vkDestroyFence(mvkLogicalDevice, mvkFencesInFlight[i], nullptr);
		}
	...
	void VulkanWindowOLD::_initPhysicalDevice(void)
	{
		Log* pLog = Log::getPointer();
	...
	int VulkanWindowOLD::_initComputePhysicalDeviceScore(VkPhysicalDevice vkPhysicalDevice)
	{
		int iScore = 0;
	...
	bool VulkanWindowOLD::update(void)
	{
		return true;
	}

	void VulkanWindowOLD::drawFrame(void)
	{
		// Wait for frames to be rendered
		vkWaitForFences(mvkLogicalDevice, 1, &mvkFencesInFlight[muiCurrentFrame], VK_TRUE, UINT64_MAX);

		// Get image from the swapchain
		uint32_t imageIndex;	// Index into mvkSwapchainImages which is now available
		VkResult result = vkAcquireNextImageKHR(mvkLogicalDevice, mvkSwapchain, UINT64_MAX, mvkSemaphoresImageAvailable[muiCurrentFrame], VK_NULL_HANDLE, &imageIndex);
		...
		{
			ThrowIfTrue(1, "VulkaanWindow::drawFrame() failed to acquire swap chain image.");
		}

		// Reset fence object to be unsignalled again...
		vkResetFences(mvkLogicalDevice, 1, &mvkFencesInFlight[muiCurrentFrame]);

		// Reset the command buffer so it can be recorded to
		vkResetCommandBuffer(mvkCommandBuffers[muiCurrentFrame], 0);

		// Record the command buffer
		_recordCommandBuffer(mvkCommandBuffers[muiCurrentFrame], imageIndex, "default", "default");

		// Submit the command buffer
		VkSubmitInfo submitInfo{};
		submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
		VkSemaphore waitSemaphores[] = { mvkSemaphoresImageAvailable[muiCurrentFrame]};
		VkPipelineStageFlags waitStages[] = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };
		submitInfo.waitSemaphoreCount = 1;
		submitInfo.pWaitSemaphores = waitSemaphores;
		submitInfo.pWaitDstStageMask = waitStages;
		submitInfo.commandBufferCount = 1;
		submitInfo.pCommandBuffers = &mvkCommandBuffers[muiCurrentFrame];
		VkSemaphore signalSemaphores[] = { mvkSemaphoresRenderFinished[muiCurrentFrame]};
		submitInfo.signalSemaphoreCount = 1;
		submitInfo.pSignalSemaphores = signalSemaphores;
		ThrowIfTrue(VK_SUCCESS != vkQueueSubmit(mvkGraphicsQueue, 1, &submitInfo, mvkFencesInFlight[muiCurrentFrame]), "VulkanWindow::drawFrame() failed to submit draw command buffer.");
		...
		result = vkQueuePresentKHR(mvkPresentationQueue, &presentInfo);
		...
		{
			ThrowIfTrue(1, "VulkanWindow::drawFrame() failed to present swap chain image.");
		}
		muiCurrentFrame = (muiCurrentFrame + 1) % MAX_FRAMES_IN_FLIGHT;
	}

	LRESULT VulkanWindowOLD::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		switch (msg)
		{
	...
	HWND VulkanWindowOLD::getWindowHandle(void)
	{
		return mhWindowHandle;
	}

	HINSTANCE VulkanWindowOLD::getApplicationInstance(void)
	{
		return mhInstance;
	}

	void VulkanWindowOLD::setClearColour(glm::vec4& clearColour)
	{
		mv4ClearColour = clearColour;
	}

	void VulkanWindowOLD::_recreateSwapchain(void)
	{
		// Important to wait for things being used, to not be
		vkDeviceWaitIdle(mvkLogicalDevice);

		// Now cleanup the swapchain before recreating it
		// We need to cleanup the framebuffers, image views and the actual swapchain
		for (size_t i = 0; i < mvkSwapchainFramebuffers.size(); i++)
		{
			vkDestroyFramebuffer(mvkLogicalDevice, mvkSwapchainFramebuffers[i], nullptr);
		}
		for (size_t i = 0; i < mvkSwapchainImageViews.size(); i++)
		{
			vkDestroyImageView(mvkLogicalDevice, mvkSwapchainImageViews[i], nullptr);
		}
		vkDestroySwapchainKHR(mvkLogicalDevice, mvkSwapchain, nullptr);
